...
% IP
rng(1)
ip{1} = .5*(rand(40,1)-.5+1i*.5*rand(40,1)); % then complex conjugated
ip{1} = ip{1}(2:end-1);
ip{2} = 18:52;%
% interlace
for ii = 1:numel(ip)
...
if SAVEIT; drawnow, mlf.figSavePNG('svd_rnd2',.5), pause(.5); end
...
for ii = 1:numel(pSpace)
    ...
    if SAVEIT; mlf.saveGIF(handler,ii,['ex_rnd2_' num2str(2) '_min' num2str(min(pSpace)) 'max' num2str(max(pSpace)) ]); end
end
...
for ii = 1:numel(pSpace)
    ...
    drawnow, 
    if SAVEIT; mlf.saveGIF(handler,ii,['ex_rnd2_eval_' num2str(2) '_min' num2str(min(pSpace)) 'max' num2str(max(pSpace)) ]); end
end
