...
Hf  = @(x1,x2) H([x1,x2]);
% IP
%ip{1} = 5*(-1+1*exp(1i*linspace(0,2e-2,22))); % then complex conjugated
ip{1} = 0+1i*linspace(0,.075,22); % then complex conjugated
ip{1} = ip{1}(2:end-1);
ip{2} = 18:52;%
...
if SAVEIT; drawnow, mlf.figSavePNG('svd_iR',.5), pause(.5); end
